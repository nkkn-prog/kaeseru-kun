...
汎用 UI コンポーネント（Button, TextInput, Card, Modal など）は Mantine UI を使う。
独自コンポーネントをゼロから作らず、Mantine をベースにカスタマイズする方針。

### Prisma

DB アクセスはすべて Prisma 経由で行う。Supabase クライアントは Auth・Storage のみに使用する。
スキーマ定義は `frontend/prisma/schema.prisma` で管理し、`docs/database-schema.md` と常に同期させること。
